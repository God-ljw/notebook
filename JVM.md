...
![image-20221103224725634](https://cdn.jsdelivr.net/gh/God-ljw/picbed@main/img3/202211032247506.png)
...
- 方法内的局部变量是否是线程安全的？
  - 如果方法内**局部变量没有逃离方法的作用范围（没有return返回值返回局部变量）**，则是**线程安全**的
...
- ```JAVA
  ...
  ![image-20221104141153615](https://cdn.jsdelivr.net/gh/God-ljw/picbed@main/img3/202211041412387.png)

  - 设置栈内存大小**-Xss内存** 例如-Xss256k
...
**2.多个线程发生死锁（程序等等很久都没有得到想要的结果）**

- 查看线程的端口号 再使用jstack查看进程id 到最后的查询结果看错误的位置。
...
- ```java
  ...
  // 线程1在锁住a时休眠，再此期间线程2锁住b，然后一段时间后线程1想要获取b锁，但是线程2已经把b锁住了，而线程2也想获取锁a，由于线程1要获取锁b，所以锁a未被释放导致死锁
...
通过new关键字**创建的对象**都会被放在堆内存，设置堆内存大小**-Xmx内存 例如-Xmx8m**
...
- **jps**命令
  - 查看当前系统中有哪些java进程（获取进程id和对于的运行情况）

- **jmap**命令
  - 只能查看某一时刻堆内存占用情况 jmap - heap 进程id

- **jconsole**工具
  - 图形界面的，多功能的监测工具，可以连续监测

![image-20221104151631011](https://cdn.jsdelivr.net/gh/God-ljw/picbed@main/img3/202211041516912.png)



- 案例：**在进行垃圾回收后，内存占用任然很高**
  - **jvirsalvm**命令开启图形化工具**注意jdk1.8后没有此工具需要自己安装**
  - 使用图形工具中的**堆转储 dump**抓取内存快照
...
public class Demo1_22 {
    ...
    public static void main(String[] args) {
        ...
        String s2 = "b";
        String s3 = "ab";
        String s4 = s1 + s2; // 先new StringBuilder()然后.append("a").append("b").toString()  最后new String("ab") 放到常量池中去 --> new 出来的放在堆里面 堆存放地址值然后指向常量池
...
https://blog.csdn.net/weixin_50280576/article/details/113532093