...
![img](https://cdn.nlark.com/yuque/0/2022/png/12843528/1652007658048-254e30ad-7754-4d3f-bac8-9db76394aa92.png)
...
![img](https://cdn.nlark.com/yuque/0/2022/png/12843528/1652009765445-b671ec98-94b9-42b7-8a46-e772845eb80c.png)
...
![img](https://cdn.nlark.com/yuque/0/2022/png/12843528/1652065909188-b365695f-a66c-4230-a32b-a0fa650044e3.png)
...
![img](https://cdn.nlark.com/yuque/0/2022/png/12843528/1652076752028-162f1829-64c3-46ed-9d16-694b4a7503af.png)
...
![img](https://cdn.nlark.com/yuque/0/2022/png/12843528/1652077077802-b6756029-3499-4a23-9fde-45edc64ab040.png)
...
B. 	查询所有员工 emp 及其所属领导的名字 emp , 如果员工没有领导, 也需要查询出来 
...
- 对于联合查询的多张表的字段数量必须保持一致，每个字段类型也需要保持一致。 
...
![img](https://cdn.nlark.com/yuque/0/2022/png/12843528/1652078546914-6802d4e8-e54f-42e3-8083-32cfaf8c75ac.png)
...
![img](https://cdn.nlark.com/yuque/0/2022/png/12843528/1652078654754-4be28ccf-b664-47e2-97de-c912df6d5dd4.png)
...
![img](https://cdn.nlark.com/yuque/0/2022/png/12843528/1652078758234-8cca56ba-0e8a-4631-b6f0-e46be70c89ff.png)
...
![img](https://cdn.nlark.com/yuque/0/2022/png/12843528/1652079099932-f794bedc-7100-40b9-860b-e31271df5a2d.png)
...
select * 
from emp 
where salary > any ( select salary from emp where dept_id = 
...
select avg(e.salary) 
from emp e, dept d 
where e.dept_id = d.id and d.name = '研发 部';
```
...
事务 是一组操作的集合，它是一个不可分割的工作单位，事务会把所有的操作作为一个整体一起向数据库系统提交或撤销操作请求，即这些操作要么同时成功，要么同时失败。



就比如: 张三给李四转账1000块钱，张三银行账户的钱减少1000，而李四银行账户的钱要增加1000。 这一组操作就必须在一个事务的范围内，要么都成功，要么都失败。

![img](https://cdn.nlark.com/yuque/0/2022/png/12843528/1652080476691-265795ef-5077-4539-884f-6e76083ddd17.png)

正常情况: 转账这个操作, 需要分为以下这么三步来完成 , 三步完成之后, 张三减少1000, 而李四增加1000, 转账成功 : 

![img](https://cdn.nlark.com/yuque/0/2022/png/12843528/1652080510295-156667db-d925-4fef-a3d6-ab4819c3829b.png)

异常情况: 转账这个操作, 也是分为以下这么三步来完成 , 在执行第三步时报错了, 这样就导致张三减少1000块钱, 而李四的金额没变, 这样就造成了数据的不一致, 就出现问题了。

![img](https://cdn.nlark.com/yuque/0/2022/png/12843528/1652080533460-c45b0aec-a273-41aa-8dd3-a57c9c4dd9fd.png)

为了解决上述的问题，就需要通过数据库的事务来完成，我们只需要在业务逻辑执行之前开启事务，执行完毕后提交事务。如果执行过程中报错，则回滚事务，把数据恢复到事务开启之前的状态。

![img](https://cdn.nlark.com/yuque/0/2022/png/12843528/1652080625037-9baa3a3f-acaf-4641-a848-01ef30b9bf11.png)
...
![img](https://cdn.nlark.com/yuque/0/2022/png/12843528/1652080736243-d1c0d876-5d95-4a42-afbe-8278c8029354.png)
...
![img](https://cdn.nlark.com/yuque/0/2022/png/12843528/1652080790588-15ba44d6-0b5e-437d-a80e-0f5d58d0b30b.png)
...
**注意：上述的这种方式，我们是修改了MySQL数据库事务的自动提交行为, 把默认的自动提交修改为了手动提交, 此时我们执行的DML语句后都不会提交, 需要手动的执行commit进行提交。**
...
- 隔离性（Isolation）：数据库系统提供的隔离机制，保证事务在不受外部并发操作影响的独立环境下运行。 我们可以给数据库系统设置事务的隔离级别。
- 持久性（Durability）：事务一旦提交或回滚，它对数据库中的数据的改变就是永久的。

上述就是事务的四大特性，简称ACID。 

![img](https://cdn.nlark.com/yuque/0/2022/png/12843528/1652081117085-0ac48eb3-2ec1-46f3-a3e0-c0508ce5c76e.png)
...
### 1). 赃读：一个事务中读到另外一个事务修改但未提交的数据。

![img](https://cdn.nlark.com/yuque/0/2022/png/12843528/1652081251014-0a969a44-945e-412b-a724-3f43096db09a.png)

比如事务B读取到了事务A更新但未提交的数据。



### 2). 不可重复读：一个事务中先后读取同一条记录，但两次读取的数据不同，称之为不可重复读。

![img](https://cdn.nlark.com/yuque/0/2022/png/12843528/1652081391357-87dc5a92-e2f8-4512-ab9c-6f90560271ed.png)

事务A两次读取同一条记录，但是读取到的数据却是不一样的。



### 3). 幻读：一个事务中按照条件查询数据时，没有对应的数据行，但是在插入数据时，又发现这行数据已经存在，好像出现了 "幻影"。 

![img](https://cdn.nlark.com/yuque/0/2022/png/12843528/1652081483450-604b0c9e-a4e9-4554-9996-d0e33fc56061.png)
...
![img](https://cdn.nlark.com/yuque/0/2022/png/12843528/1652081737597-6c6ef995-935c-4078-ae71-2b47ed860928.png)
