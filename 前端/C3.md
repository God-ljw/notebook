...
```HTML
<p class="class-name1 class-name2"></p>Copy to clipboardErrorCopied
```
...
```css
text-decoration: underline;Copy to clipboardErrorCopied
```
...
```html
<div style="color: red; font-size: 12px">行内样式表</div>Copy to clipboardErrorCopied
```
...
```html
<link rel="stylesheet" href="css文件路径" />Copy to clipboardErrorCopied
```

[
  ](https://docs.mphy.top/#/CSS/ch02)
...
```CSS
元素1 > 元素2 { 样式声明; }Copy to clipboardErrorCopied
```
...
a:active /*选择活动链接（鼠标按下未弹起的链接）*/Copy to clipboardErrorCopied
```
...
```css
background-color: 颜色值;Copy to clipboardErrorCopied
```
...
```css
background-repeat: repeat(默认) | no-repeat | repeat-x | repeat-y;Copy to clipboardErrorCopied
```
...
```css
background-position: x y;Copy to clipboardErrorCopied
```
...
```css
background-attachment: scroll | fixed;Copy to clipboardErrorCopied
```
...
```css
background: transparent url(image.jpg) repeat-y fixed top;Copy to clipboardErrorCopied
```
...
```css
background: rgba(0, 0, 0, 0.3);Copy to clipboardErrorCopied
```
...
```css
/*属性可连写*/
border: border-width || border-style || border-color;Copy to clipboardErrorCopied
```
...
```css
/*习惯顺序*/
border: 5px solid pink;Copy to clipboardErrorCopied
```
...
```css
/*注意层叠性*/
border-top: 1px solid red;Copy to clipboardErrorCopied
```
...
```css
border-collapse: collapse;Copy to clipboardErrorCopied
```
...
- ```
  ...
  ```css
  box-sizing: border-box;Copy to clipboardErrorCopied
  ```
...
```css
text-align: center;Copy to clipboardErrorCopied
```
...
```css
border-radius: length;Copy to clipboardErrorCopied
```
...
```css
bxo-shadow: h-shadow v-shadow blur spread color inset;Copy to clipboardErrorCopied
```
...
```css
cursor: default | pointer | move | text | not-allowed;Copy to clipboardErrorCopied
```
...
overflow: hidden;
/*3.文字用省略号替代超出的部分*/
text-overflow: ellipsis;Copy to clipboardErrorCopied
```
...
<input type="password">
<input type="text">Copy to clipboardErrorCopied
```
